...
(use-package mu4e
  ...
  (setq user-mail-address "codybense@gmail.com"
        user-full-name "Cody Bense")
        ...
        (list
         (make-mu4e-context
          ...
          :match-func (lambda (msg)
                        (when msg
                          (string-prefix-p "/codyGmail"
                                           (mu4e-message-field msg :maildir))))
          :vars '((user-mail-address  . "codybense@gmail.com")
                  (user-full-name     . "Cody Bense")
                  (mu4e-sent-folder   . "/codyGmail/Sent")
                  (mu4e-drafts-folder . "/codyGmail/Drafts")
                  (mu4e-trash-folder  . "/codyGmail/Trash")
                  (mu4e-refile-folder . "/codyGmail/Archive")))

         (make-mu4e-context
          :name "CodyProton"
          :match-func (lambda (msg)
                        (when msg
                          (string-prefix-p "/CodyProton"
                                           (mu4e-message-field msg :maildir))))
          :vars '((user-mail-address  . "codybense@proton.me")
                  (user-full-name     . "Cody Bense")
                  (mu4e-sent-folder   . "/codyProton/Sent")
                  (mu4e-drafts-folder . "/codyProton/Drafts")
                  (mu4e-trash-folder  . "/codyProton/Trash")
                  (mu4e-refile-folder . "/codyProton/Archive")))))
  ...
  (setq mu4e-bookmarks
        '(("flag:unread AND NOT flag:trashed" "Unread messages"   ?u)
          ...
          ("maildir:/CodyGmail/Inbox"    "Personal Inbox"    ?p)
